        ...
        Try
            ...
            If xxx = 0 Then
                ...
                If pocett < 0 Then
                    Dim aks As String
                    Dim ifg As Integer
                    aks = "Má ich už len: " & DataGridView2.Rows(0).Cells(6).Value & ". Naozaj má vrátiť až " & -pocet & "?"
                    ifg = MsgBox(aks, vbExclamation + vbYesNo, "Overenie")
                    If ifg = vbYes Then
                        pocett = 0
                    Else
                ...
                If DataGridView2.RowCount = 1 Then
                    Sql = "UPDATE Rotek SET Kolko='" & pocett & "' WHERE Nástroj='" + nazov + "' AND pocet=" & typ & " AND VelkostR='" & ident & "' AND Menpr='" + crc + "'"
                    cmd = New SqlCommand(Sql, con)
                    cmd.ExecuteNonQuery()
                Else
                    Sql = "Insert INTO Rotek (Nástroj, pocet, Menpr, Kolko, Spolu, VelkostR, srot, srotcena, Meno) VALUES ('" + nazov + "', '" & typ & "', '" + crc + "', '" & pocet & "', '" & 0 & "', '" + ident + "', '" & 0 & "', '" & 0 & "','" + cesta + "')"
                    cmd = New SqlCommand(Sql, con)
                    cmd.ExecuteNonQuery()
                End If
                con.Close()
