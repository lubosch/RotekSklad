...
Public Class Pridat_CPO
    Public co As String

    Private Sub Pridat_CPO_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            If String.IsNullOrEmpty(co) Then
                Me.COBindingSource1.Filter = String.Format("{0} = '{1}' AND {2} LIKE '{3}%' AND {4} LIKE '%{5}' ", RotekDataSet.CO.pocetColumn, 1, RotekDataSet.CO.NazovColumn, "OB ", RotekDataSet.CO.NazovColumn, "/" & rok())
                Me.COBindingSource.Filter = String.Format("{0} = '{1}' AND {2} LIKE '{3}%' AND {4} LIKE '%{5}' ", RotekDataSet.CO.pocetColumn, 1, RotekDataSet.CO.NazovColumn, "co fhfghfghfhfg564r98sx", RotekDataSet.CO.NazovColumn, "/" & rok())
                Me.COBindingSource1.Sort = "Nazov DESC"
                If DataGridView2.RowCount = 0 Then
                    TextBox1.Text = "OB 0001/" & rok()
                Else
                    Dim co As String = DataGridView2.Rows(0).Cells(1).Value
                    co = co.Replace("OB ", "")
                    co = co.Replace("/" & rok(), "")
                    Try
                        Dim i As Integer = co
                        TextBox1.Text = "OB " & Format(i + 1, "0000") & "/" & rok()
            ...
            Else
                Try
                    Me.COBindingSource.Filter = String.Format("{0} = '{1}' AND {2} = '{3}' ", RotekDataSet.CO.pocetColumn, 2, RotekDataSet.CO.NazovColumn, co)
                    TextBox1.Text = co
                    Me.COBindingSource1.Filter = String.Format("{0} = '{1}' AND {2} = '{3}'", RotekDataSet.CO.pocetColumn, 1, RotekDataSet.CO.NazovColumn, co)
    ...
    Private Function Vseobecne() As Boolean
        Try
            ...
            If DataGridView5.RowCount < 1 And ComboBox2.Text.Length > 0 Then
                If MsgBox("Nenašiel sa u nich taký zamestnanec. Chcete pridať nového? ", vbQuestion + vbYesNo, "Neexistuje") = vbYes Then
            ...
            Me.COBindingSource1.Filter = String.Format("{0} = '{1}' AND {2}='{3}' ", RotekDataSet.CO.pocetColumn, 1, RotekDataSet.CO.NazovColumn, nazov)
            If DataGridView2.RowCount <> 0 And Label7.Enabled = False Then
                Chyby.Show("Už existuje")
                con.Close()
                Return False
            ElseIf DataGridView2.RowCount <> 0 Then
                If TextBox1.Text = Me.co Then
                    sql = "UPDATE CO SET Firma='" & ComboBox1.Text & "', Veduci='" & ComboBox2.Text & "', Poznamka='" & TextBox3.Text & "', Datum='" & datum.ToString("yyyy-MM-dd") & "', Popis='" & popis & "' WHERE pocet=1 AND Nazov='" & nazov & "'"
                    cmd = New SqlCommand(sql, con)
                    cmd.ExecuteNonQuery()
                Else
                    Chyby.Show("Už existuje")
            ...
            ElseIf Label7.Enabled = True Then
                sql = "UPDATE CO SET Nazov='" & nazov & "' WHERE Nazov='" & Me.co & "'"
                Me.co = nazov
            Else
                sql = "INSERT INTO CO (Nazov, Popis, Firma, Veduci, Datum, Cena, Poznamka, pocet, Evidoval) VALUES('" & nazov & "','" & popis & "','" & firma & "','" & veduci & "','" & datum.ToString("yyyy-MM-dd") & "','" & 0 & "','" & poznamka & "'," & 1 & ",'" & Form78.uzivatel & "')"
            End If
    ...
    Private Sub Polozka()
        ...
        Try
            ...
            Dim sql As String
            Me.COBindingSource1.Filter = String.Format("{0} = '{1}' AND {2}='{3}' AND {4}='{5}'", RotekDataSet.CO.pocetColumn, 2, RotekDataSet.CO.PolozkaColumn, polozka, RotekDataSet.CO.NazovColumn, nazov)
            If DataGridView2.RowCount <> 0 Then
                cena1 = cena1 - DataGridView2.Rows(0).Cells(0).Value * DataGridView2.Rows(0).Cells(8).Value
                sql = "UPDATE CO SET Cena='" & cena & "', Kusov='" & kusov & "', Evidoval='" & Form78.uzivatel & "' WHERE Nazov='" & nazov & "' AND Polozka='" & polozka & "' AND pocet=" & 2
                'Chyby.Show("Už je položka pridaná")
            Else
                sql = "INSERT INTO CO (Nazov, Polozka, Kusov, Datum, Cena, pocet, Evidoval) VALUES('" & nazov & "','" & polozka & "','" & kusov & "','" & datum.ToString("yyyy-MM-dd") & "','" & cena & "'," & 2 & ",'" & Form78.uzivatel & "')"
            End If
            Form78.sqa(sql)

            sql = "UPDATE CO SET Cena='" & cena1 & "' WHERE pocet=1 AND Nazov='" & nazov & "'"
            Form78.sqa(sql)

            Me.COTableAdapter.Fill(Me.RotekDataSet.CO)
            Me.COBindingSource.Filter = String.Format("{0} = '{1}' AND {2}='{3}'", RotekDataSet.CO.pocetColumn, 2, RotekDataSet.CO.NazovColumn, nazov)
            napln()
    ...
    Private Sub ComboBox1_TextChanged(sender As System.Object, e As System.EventArgs) Handles ComboBox1.TextUpdate
        Dim tex As String = ComboBox1.Text
    ...
    Private Sub TextBox1_Leave(sender As System.Object, e As System.EventArgs) Handles TextBox1.Leave
        Dim co As String = TextBox1.Text
        If (co.IndexOf("/" & rok()) = co.Length - 3) Then
            co = co.Replace("/" & rok(), "")
            co = co.Replace("OB ", "")
            co = co.Replace("OB", "")
            Dim i As Integer
            Try
                i = co
                TextBox1.Text = "OB " & Format(i, "0000") & "/" & rok()
            Catch ex As Exception
            End Try
        End If
    End Sub
    ...
    Private Sub DataGridView1_CellContentClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView1.CellClick
        If e.RowIndex = -1 Or Label7.Enabled = False Then
            Exit Sub
        End If
        If e.ColumnIndex = 4 Then
            ...
            Try
                ...
                Me.COBindingSource1.Filter = String.Format("{0} = '{1}' AND {2}='{3}'", RotekDataSet.CO.pocetColumn, 1, RotekDataSet.CO.NazovColumn, TextBox1.Text)
            ...
            Dim sql As String
            sql = "DELETE FROM co WHERE pocet=2 AND Nazov='" & TextBox1.Text & "' AND Polozka='" & DataGridView1.Rows(e.RowIndex).Cells(0).Value & "'"
            Form78.sqa(sql)
            sql = "UPDATE CO SET Cena='" & cena & "' WHERE pocet=1 AND Nazov='" & TextBox1.Text & "'"
            Form78.sqa(sql)
